...
#q30 filter
find . -name "*sam" | xargs -n 1 -P $threads -iFILES sh -c '
 samtools view -F 1548 -h -S -q 30 -o FILES_q30.sam FILES;
';

#generate tag densities for q30
find . -name "*_q30.sam" | xargs -n 1 -P $threads -iFILES sh -c '
...
find . -name "*rep1*.tagAlign.gz" | xargs -n 1 -P $threads -iFILES sh -c '
 r1=FILES; 
 r2=$(echo $r1 | sed "s/rep1/rep2/"); 
 r3=$(echo $r1 | sed "s/rep1/rep3/"); 
 r0=$(echo $r1 | sed "s/rep1/rep0/"); 
 zcat $r1 $r2 $r3 | gzip -c > $r0;
';

#shuffle tags and generate psuedo replicates
find . -name "*tagAlign.gz"  -not -name "*input*" | xargs -n 1 -P $threads -iFILES sh -c '
...
#macs2 peak calls
find . -name "*tagAlign*gz"  -not -name "*input*" -not -name "*VS*" | xargs -n 1 -P $threads -iFILES sh -c '
 rep=$(echo FILES | sed "s/\.tagAlign.*//g"); 
 inp=$(echo FILES | sed "s/rep./input\.rep0/"); 
 ss=$(cat FILES.cc | grep FILES | cut -f3 | head -n 1 | cut -d "," -f1); 
 macs2 callpeak -t FILES -c $inp -g $gsize --to-large --nomodel --extsize $ss -p 0.1 -n "$rep"_VS_input;
';

# sort MACS2 peaks
find . -name "*.narrowPeak" | xargs -n 1 -P $threads -iFILES sh -c '
 sort -k 8nr,8nr FILES | head -n 30000 > FILES.sorted;
';
